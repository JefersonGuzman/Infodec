<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="text-dark mb-0">
        <i class="bi bi-upload me-2"></i>Gestionar Ventas
    </h2>
</div>
...
<div class="card mb-4">
    <div class="card-header bg-light">
        <h5 class="card-title mb-0 text-dark">
            <i class="bi bi-file-earmark-arrow-up me-2"></i>Cargar Archivo CSV de Ventas
        </h5>
    </div>
    <div class="card-body">
        <form method="post" enctype="multipart/form-data" action="index.php?controller=Ventas&action=upload">
            <div class="row">
                <div class="col-md-8">
                    <input type="file" name="csvfile" accept=".csv" class="form-control" required>
                    <div class="form-text text-muted">
                        Seleccione un archivo CSV con formato: FechaVenta, Vendedor, Producto, Referencia, Cantidad, ValorUnitario, ValorVendido, Impuesto
                    </div>
                </div>
                <div class="col-md-4">
...
<div class="card">
    <div class="card-header bg-light">
        <h5 class="card-title mb-0 text-dark">
            <i class="bi bi-table me-2"></i>Registros de Ventas
        </h5>
    </div>
    <div class="card-body p-0">
        <?php if (empty($ventas)): ?>
            <div class="text-center py-5">
                ...
                <p class="text-muted mt-3">No hay registros de ventas disponibles</p>
            </div>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($ventas as $venta): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($venta['referencia']); ?></td>
                                <td><?php echo number_format($venta['cantidad']); ?></td>
                                <td>$<?php echo number_format($venta['valor_unitario'], 0, ',', '.'); ?></td>
                                <td>$<?php echo number_format($venta['valor_vendido'], 0, ',', '.'); ?></td>
                                <td>$<?php echo number_format($venta['impuesto'], 0, ',', '.'); ?></td>
                                <td>
                                    <a href="index.php?controller=Ventas&action=delete&id=<?php echo $venta['id']; ?>" 
